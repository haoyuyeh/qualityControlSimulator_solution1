...
public class Belt {
	...
	public synchronized Bicycle get(int index, String message)
		...
		// make a note of the event in output trace
		System.out.println(
				indentation + indentation + bicycle + " is fetched by robot");
	...
	public synchronized Bicycle getEndBelt()
		...
		if (bicycle.isDefective() && !bicycle.isInspected()) {
			...
			// make a note of the event in output trace
			System.out
					.println(indentation + indentation + bicycle + " departed");
	...
	public synchronized void move()
		...
		// or segment 3 is occupied by a bike which is tagged and not inspected,
		// do not move the belt
		while (isEmpty() || sensor() || segment[segment.length - 1] != null) {
			wait();
		}
